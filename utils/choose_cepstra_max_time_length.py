...
def choose_cepstra_max_frame_length(cepstra_load_dir, transcript_load_dir, cepstra_save_dir, transcript_save_dir,
    ...
    for i in range(len(cepstra)):
        ...
        for cepstrum, cepstrum_path, label_path in zip(cepstra[i], cepstra_paths[i], label_paths[i]):
            if cepstrum.shape[0] <= max_frame_length:
...
if __name__ == '__main__':
    max_frame_length = 3000
    load_dir = "../data/train"
    save_dir = "../data/train_max_{}".format(max_frame_length)

    choose_cepstra_max_frame_length(load_dir, load_dir, save_dir, save_dir, max_frame_length)